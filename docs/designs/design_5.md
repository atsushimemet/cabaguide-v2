...
## 1. 背景
- 現状は `input[type="number"]` で来店人数/指名キャスト数/キャストドリンク数/延長回数を入力しているが、スマホで 0 を消して別の数字にしようとするとキーボード操作が煩雑になる。
...
- 予算計算結果 (`guestCount`, `nominationCount`, `castDrinkCountPerGuest`, `extensionCount`) がセレクト値に連動して更新される。
