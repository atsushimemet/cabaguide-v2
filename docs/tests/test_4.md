...
- **TC-4-5: 指名料/キャストドリンク/サービス料/税の適用順序**  
  前提: `nomination_price = 3000`, `light_drink_price = 2500`, `service_fee_rate = 0.25`, `tax_rate = 0.1` の `StoreBasePricing`。指名人数 2、キャストドリンク 3 杯、来店人数 2 で計算する。  
  手順: `calculateBudget` に上記パラメータを渡して計算する。  
  期待結果: 小計に 2h (連続する 1h スロット ×2) の時間帯料金 + 指名料 (3000 × 2 × 2h) + ドリンク (2500 × 3) が順に加算され、その後 `service_fee_rate`、最後に `tax_rate` を乗算した総額になる。`light_drink_price` が未設定の場合は 2,000 円固定値にフォールバックすることも合わせて確認する。

- **TC-4-6: 延長料金の開口部確認**  
  前提: `extension_price = 6000`。延長回数 2 回を入力できる UI/関数引数がある。  
  手順: 延長回数を 2 にして `calculateBudget` を実行し、同条件で延長 0 のケースと比較する。  
  期待結果: 延長 2 回のケースでは小計に `extension_price * 2` が追加され、その他の計算ロジックは影響を受けない。
